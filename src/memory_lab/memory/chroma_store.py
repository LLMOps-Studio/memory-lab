...
class LongTermMemory:
    ...
    def save_fact(self, user_id: str, fact: str):
        ...
        self.collection.upsert(
            documents=[fact], metadatas=[{"user_id": user_id}], ids=[fact_id]
        )
        print(f"[Memory] Saved new fact for {user_id}: {fact}")

    def retrieve_relevant_facts(
        self, user_id: str, query: str, n_results: int = 3
    ) -> list[str]:
        ...
        try:
            ...
            return []
        # A ChromaDB outage shouldn't crash the chat request -- degrade to
        # no recalled facts instead.
